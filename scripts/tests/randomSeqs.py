...
def outputGenerated(generated, rna=False):
    ...
    print("Adenine: %s out of %s (%s%%)" % (ntctr["A"], totctr, (100*float(ntctr["A"])/float(totctr))))
    print("Guanine: %s out of %s (%s%%)" % (ntctr["G"], totctr, (100*float(ntctr["G"])/float(totctr))))
    print("Cytosine: %s out of %s (%s%%)" % (ntctr["C"], totctr, (100*float(ntctr["C"])/float(totctr))))
    if rna:
        print("Uracil: %s out of %s (%s%%)" % (ntctr["U"], totctr, (100*float(ntctr["U"])/float(totctr))))
    else:
        print("Thymine: %s out of %s (%s%%)" % (ntctr["T"], totctr, (100*float(ntctr["T"])/float(totctr))))
